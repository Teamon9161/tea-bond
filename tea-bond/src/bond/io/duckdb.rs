...
impl TryFrom<&Row<'_>> for WindSqlRow {
    ...
    fn try_from(value: &Row<'_>) -> std::result::Result<Self, Self::Error> {
        Ok(WindSqlRow {
            ...
            b_info_interesttype: value.get::<_, Option<f64>>("b_info_interesttype")?.map(|v| v as i32),
            b_info_couponrate: value.get("b_info_couponrate")?,
            b_info_spread: value.get("b_info_spread")?,
            b_info_interestfrequency: value.get::<_, Option<Arc<str>>>("b_info_interestfrequency")?.map(|s| s.as_ref().into()),
            b_info_carrydate: NaiveDate::parse_from_str(value.get::<_, Arc<str>>("b_info_carrydate")?.as_ref(), "%Y%m%d").expect("Parse b_info_carrydate failed"),
            b_info_maturitydate: NaiveDate::parse_from_str(value.get::<_, Arc<str>>("b_info_maturitydate")?.as_ref(), "%Y%m%d").expect("Parse b_info_maturitydate failed"),
